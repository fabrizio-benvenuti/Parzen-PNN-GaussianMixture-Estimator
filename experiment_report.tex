...
These results can be interpreted as a form of \textbf{implicit regularization}: while KDE directly
interpolates the sample set through local kernels, the PNN must represent the density through a finite
parametric function class. This architectural constraint can smooth sampling noise, which is reflected
in improved held-out NLL in the more complex mixtures.
...
\subsubsection*{Findings}
An ablation on Mixture~3 (MLP [30,20] with ReLU output and log-density training at $h_1=12$) shows that
adding uniform supervision slightly improves held-out NLL (from $\approx 4.103$ without uniform points to
$\approx 4.088$ with uniform points) and reduces the mean predicted density near the boundary of $D$
(boundary-ring mean $\approx 2.341\times 10^{-3}$ to $\approx 2.316\times 10^{-3}$).
This supports the intended effect: \textbf{uniform supervision reduces far-field artifacts} by anchoring
the model to Parzen targets not only at samples but also in empty regions of the domain. In practical
terms, this is evidence that the PNN is not merely memorizing sample locations, but is learning the
structure of the density field in regions with little or no data.
...
uniform targets. Uniform supervision reduces spurious density away from the samples and slightly
improves held-out NLL.}
...
\begin{itemize}
    ...
    indicating a small artificial bias: the penalty explicitly pushes the learned density toward zero near the
    boundary. This can conflict with the maximum-likelihood objective on the data, especially when samples lie
    near the boundaries of $D$; therefore the observed NLL increase is expected under this selection criterion.
...
\begin{itemize}
    ...
    \item \textbf{Uniform supervision reduces extrapolation artifacts:} adding interior uniform targets slightly improves held-out
    NLL and reduces boundary-region density in Mixture~3 (Fig.~\ref{fig:uniform-supervision-m3}), consistent with the intended
    stabilization far from samples.
    ...
    \item \textbf{Model selection follows an MDL-style principle:} when validation NLL is similar across architectures, the
